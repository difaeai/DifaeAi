...
function DifaAgentAdminPageContents() {
    ...
    const fetchBehaviors = useCallback(async () => {
        if (userIdForCamera && cameraId) {
            try {
                const behaviors = await getLearnedBehaviors(userIdForCamera, cameraId);
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            if (userIdForCamera && cameraId) {
                try {
                    const prohibited = await getProhibitedObjects(userIdForCamera, cameraId);
    ...
    }, [toast, userIdForCamera, cameraId, fetchBehaviors]);
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            ...
            const savePromises = objects.map(obj => saveLearnedBehavior(userIdForCamera, cameraId!, `A prohibited object: ${obj}`));
