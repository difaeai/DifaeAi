...
const SKIP_SIGNING_VERIFICATION =
  cleanEnv(process.env.WINDOWS_AGENT_SKIP_SIGNING_VERIFICATION) === "true";
...
async function verifySignature(
  ...
  if (signer.type === "signtool") {
    args.push("verify", "/pa", "/all", executablePath);
  } else {
    args.push("verify", "-in", executablePath);
  }

  try {
    await run(signer.command, args);
  } catch (error) {
    throw new SigningExecutionError("Signed agent verification failed.", {
...
export async function maybeSignExecutable(
  ...
  try {
    ...
    await verifySignature(signer, executablePath);
  } catch (error) {
    throw new SigningExecutionError("Failed to sign Windows agent executable.", {
      cause: error as Error,
    });
  } finally {
    await certificate.cleanup();
