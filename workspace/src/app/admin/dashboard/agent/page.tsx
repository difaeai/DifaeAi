...
function DifaAgentAdminPageContents() {
    const { toast } = useToast();
    const { user } = useAuth();
    const searchParams = useSearchParams();
    const cameraId = searchParams.get('cameraId');
    const userIdForCamera = searchParams.get('userId');

    const [camera, setCamera] = useState<CameraType | null>(null);
    const [uploadedThreats, setUploadedThreats] = useState<string[]>([]);
    ...
    useEffect(() => {
        const fetchCameraDetails = async () => {
            if (userIdForCamera && cameraId) {
                const camDocRef = doc(db, 'users', userIdForCamera, 'cameras', cameraId);
        ...
        fetchCameraDetails();
    }, [userIdForCamera, cameraId, toast]);
    ...
    }, [toast, drawDetectionBox, clearDetectionBox, prohibitedObjectsInput, activeBehaviorIds, learnedBehaviors]);
    
    useEffect(() => {
        const fetchInitialData = async () => {
            if (user && userIdForCamera && cameraId) {
                try {
                    const [prohibited, behaviors] = await Promise.all([
                        getProhibitedObjects(userIdForCamera, cameraId),
                        getLearnedBehaviors(userIdForCamera, cameraId)
                    ]);
    ...
    }, [toast, user, userIdForCamera, cameraId]);
    ...
    const handleSaveBehavior = async (promptToSave: string) => {
        if (!userIdForCamera || !cameraId || !promptToSave.trim()) {
        ...
        try {
            const newBehavior = await saveLearnedBehavior(userIdForCamera, cameraId, promptToSave);
            setLearnedBehaviors(prev => [...prev, newBehavior]);
            setBehaviorPrompt("");
            toast({ title: 'Behavior Saved', description: 'The new behavior has been added to your learned behaviors.' });
        } catch (error) {
             toast({ variant: 'destructive', title: 'Save Failed', description: 'Could not save the new behavior.' });
        } finally {
    ...
    const handleDeleteBehavior = async (behaviorId: string) => {
        if (!userIdForCamera || !cameraId) return;
        try {
            await deleteLearnedBehavior(userIdForCamera, cameraId, behaviorId);
            setLearnedBehaviors(prev => prev.filter(b => b.id !== behaviorId));
    ...
    const handleSaveSettings = async () => {
        if (!userIdForCamera || !cameraId) {
            toast({ variant: 'destructive', title: 'Error', description: 'Camera or user not specified.' });
            return;
        }
        setIsSaving(true);
        try {
            const objects = prohibitedObjectsInput.split(',').map(s => s.trim()).filter(Boolean);
            await saveProhibitedObjects(objects, userIdForCamera, cameraId);
            toast({ title: 'Settings Saved', description: 'Your list of prohibited objects has been updated.' });
        } catch (error) {
    ...
    const handleAnalyzeVideo = async () => {
      ...
      try {
        ...
        if (result.isLearnedBehavior) {
            toast({
              title: "Behavior Learned!",
              description: "The new behavior has been automatically added to your list.",
            });
            // Automatically save the new behavior
            await handleSaveBehavior(result.analysis);
        } else {
             toast({
    ...
    const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
        ...
        // Simulate upload
        setTimeout(() => {
          setUploadedVideos(prev => [...prev, file.name]);
          setIsUploadingVideo(false);
          toast({ title: "Video Uploaded", description: `${file.name} is ready for training.` });
        }, 1500);
      }
    };
    ...
    <div className="space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                <div>
                    <h1 className="text-3xl font-bold font-headline">DIFAE AI Agent</h1>
                ...
                <Card>
                    ...
                    <CardDescription>
                        Use your webcam to provide a live video stream for training the AI models.
                    </CardDescription>
                    </CardHeader>
                    <CardContent>
                 ...
                 <Card>
                    ...
                    <CardContent>
                        {cameraId ? (
                            <div className="space-y-2">
                                ...
                                <p className="text-sm text-muted-foreground">User: {userIdForCamera}</p>
                            </div>
                        ) : (
                    ...
                    <CardFooter>
                        <Button asChild className="w-full">
                            <Link href="/admin/dashboard/cameras">Go to Camera Management</Link>
                        </Button>
                    </CardFooter>
                </Card>
            </div>
      ...
      <Card>
        ...
        <Card>
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    ...
                    <CardDescription>A log of all threats or persons detected from the live feed.</CardDescription>
                </div>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>A log of people identified by the AI in the live feed, with links to public social media profiles.</CardDescription>
            </CardHeader>
            <CardContent>
                <Table>
                    <TableHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the AI model responsible for detecting general threats like suspicious movement, fire, and weapons.
          </CardDescription>
        </CardHeader>
        <CardContent>
           <div className="space-y-2">
              <Label>Global Threat Detection</Label>
               <Alert>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertTitle>This is a Global Setting</AlertTitle>
                  <AlertDescription>
                   General threat detection settings are managed globally by the administrator and apply to all users.
                  </AlertDescription>
                </Alert>
           </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 font-headline">
            ...
            Object Recognition & Theft Alerts
          </CardTitle>
          <CardDescription>
            Manage settings for the object tracking and theft alert model. This feature is part of the DSG Pro and Vision plans.
          </CardDescription>
        </CardHeader>
        <CardContent>
            <Alert>
                ...
                <AlertTitle>This is a Global Setting</AlertTitle>
                <AlertDescription>
                Object recognition models are managed globally by the administrator to ensure consistency and performance across the platform.
                </AlertDescription>
            </Alert>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the facial recognition model and its billing. This feature is exclusive to the DSG Vision plan.
          </CardDescription>
        </CardHeader>
        <CardContent>
           <Alert>
                ...
                <AlertTitle>This is a Global Setting</AlertTitle>
                <AlertDescription>
                Facial recognition settings are managed globally by the administrator.
                </AlertDescription>
            </Alert>
        </CardContent>
      </Card>
...
export default function DifaAgentAdminPage() {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <DifaAgentAdminPageContents />
        </Suspense>
    );
}
